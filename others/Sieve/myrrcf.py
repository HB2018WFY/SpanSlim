...
class RCTree:
    ...
    def __init__(self, X=None, index_labels=None, precision=9, 
        ...
        if X is not None:
            ...
            self.ndim = d
            # Store the mapping from dimension to cutting point
            # self.d2q = [None] * self.ndim
    ...
    def _cut(self, X, S, parent=None, side='l'):
        ...
        # Save the cutting point whose cutting dimension is its index in d2q
        # if self.d2q[q] is None:
        #     self.d2q[q] = []
        # self.d2q[q].append(child)
    ...
    def forget_point(self, index):
        ...
        if leaf is self.root:
            ...
            self.ndim = None
            # self.d2q = None
            return self.leaves.pop(index), None
        # Find parent
        parent = leaf.u
        branch = leaf.u
        # Update d2q
        # cut_dimension = parent.q
        # self.d2q[cut_dimension].remove(parent)
        # if len(self.d2q[cut_dimension]) == 0:
        #     self.d2q[cut_dimension] = None
        # Find sibling
        if leaf is parent.l:
            sibling = parent.r
        else:
        ...
        if parent is self.root:
            ...
            return self.leaves.pop(index), branch
        # Find grandparent
        ...
        point = leaf.x
        self._relax_bbox_upwards(parent, point.x)
        return self.leaves.pop(index), branch
    ...
    def _scale_leaf(self, leaf, val=[]):
        leaf.x = np.append(leaf.x, val)
        leaf.b = leaf.x.reshape(1, -1)

    def _scale_branch(self, branch, val=[]):
        branch.b = np.append(branch.b, [val, val], axis=1)
    ...
    def insert_point(self, point, index, tolerance=None):
        ...
        >>> tree.insert_point(x, index=0)
        """
        if not isinstance(point.x, np.ndarray):
            point.x = np.asarray(point.x)
        point.x = point.x.ravel()
        if self.root is None:
            leaf = Leaf(x=point, i=index, d=0)
            self.root = leaf
            self.ndim = point.x.size
            # self.d2q = [None] * self.ndim
        ...
        # If point has new dimension, scale the leaves and branches to the same dimension
#         if point.size > self.ndim:
#             inc = point.size - self.ndim
#             self.ndim += inc
#             self.map_branches(self.root, op=self._scale_branch, inc=inc)
#             self.map_leaves(self.root, op=self._scale_leaf, inc=inc)
#             # Extend the d2q
#             self.d2q.extend([None] * inc)

        # Check for duplicate points
        duplicate = self.find_duplicate(point.x, tolerance=tolerance)
        if duplicate:
        ...
        for _ in range(maxdepth + 1):
            bbox = node.get_bbox()
            cut_dimension, cut = self._insert_point_cut(point.x, bbox)
        ...
        self.leaves[index] = leaf
        # Return inserted leaf and branch for convenience
        return leaf, branch
    ...
    def codisp(self, leaf):
        ...
        '''
        #return co_displacement + self.maxDepth() / leaf.d
        return self.maxDepth() / leaf.d
    ...
    def find_duplicate(self, point, tolerance=None):
        ...
        if tolerance is None:
            if (nearest.x.x == point).all():
                return nearest
        else:
            if np.isclose(nearest.x.x, point, rtol=tolerance).all():
                return nearest
        return None
    ...
    def _lr_branch_bbox(self, node):
        ...
        """
        lbbox = node.l.get_bbox()
        rbbox = node.r.get_bbox()
        '''
        if isinstance(node.l, Leaf):
            lb1 = node.l.x.x
            lb2 = node.l.x.x
        else:
            lb1 = node.l.b[0, :]
            lb2 = node.l.b[-1, :]
        if isinstance(node.r, Leaf):
            rb1 = node.r.x.x
            rb2 = node.r.x.x
        else:
            rb1 = node.r.b[0, :]
        ...
        bbox = np.vstack([np.minimum(lbbox[0, :], rbbox[0, :]), np.maximum(lbbox[-1, :], rbbox[-1, :])])
        return bbox
    ...
    def _insert_point_cut(self, point, bbox):
        ...
        '''
#         invalid = np.logical_and(b_span > bbox_hat[-1, :], b_span > 0)
        newDim = np.logical_and(bbox[-1, :] == -1, point != -1)
        invalidDim = np.logical_and(bbox[0, :] != -1, point == -1)
        found = np.logical_or(newDim, invalidDim)
        if found.any():
#             for j in range(invalid.size):
#                 if invalid[j]:
            for j in range(found.size):
                if found[j]:
                    cut_dimension = j
                    cut = -0.5
                    break
    ...
    '''
    def find_weak_dimensions(self):
        data = np.array([leaf.x for leaf in self.leaves.values()])
        weak_dimensions = []
        if data.shape[0] < 2:
            return weak_dimensions
        try:
            for i in range(data.shape[1] - 1, -1, -1):
                if data[:, i].var() < 0.1 and self.d2q[i] == None:
    ...
    def compact(self, weak_dimensions=None):
        '''        
        try:
        ...
        # Set the cutting dimension of the cutting point
        try:
            for i in range(weak_dimensions[-1], len(self.d2q)):
                if self.d2q[i] is not None:
                    for b in self.d2q[i]:
                        b.q = i
        except IndexError:
            set_trace()
        ...
        # Remove the weak dimension of the data point
        for leaf in self.leaves.values():
            if len(leaf.x) == self.ndim:
                leaf.x = np.delete(leaf.x, weak_dimensions)
            # Update the bbox of the leaf
        for leaf in self.leaves.values():
            leaf.b = leaf.x.x.reshape(1, -1)
        self.ndim -= len(weak_dimensions)
        # Update the bbox from root to leaf
        # self._get_bbox_top_down(self.root)

    def extend(self, values):
        if self.ndim:
            self.ndim += len(values)
            self.map_branches(self.root, op=self._scale_branch, val=values)
            self.map_leaves(self.root, op=self._scale_leaf, val=values)
            # Extend the d2q
            # self.d2q.extend([None] * len(values))
...
#         self.b = x.reshape(1, -1)
    ...
    def get_bbox(self):
        return np.vstack([self.x.x, self.x.x]) 
