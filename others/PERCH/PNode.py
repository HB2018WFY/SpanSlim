...
#     if len(x) > len(y):
#         padNum = len(x) - len(y)
#         y = np.pad(y,(0,padNum),'constant', constant_values=(0,0)) 
#     elif len(y) > len(x):
#         padNum = len(y) - len(x)
#         x = np.pad(x,(0,padNum),'constant', constant_values=(0,0))
#     return x, y
...
class PNode:
    ...
    def max_distance(self, x):
        ...
        """
        if self.pts and self.point_counter == 1:
            # tmp_x, tmp_y = align(x, self.pts[0][0])
            # return _fast_norm(tmp_x-tmp_y)
